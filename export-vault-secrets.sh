...
then
    echo "Setting DOKOPP_CERT_KEYSTORE"
    ...
    export DOKOPP_CERT_KEYSTORE=$CERT_PATH
fi

if test -f /var/run/secrets/nais.io/certificate/srvdokopp/keystorealias
then
    echo "Setting DOKOPP_CERT_KEYSTOREALIAS"
    export DOKOPP_CERT_KEYSTOREALIAS=$(cat /var/run/secrets/nais.io/certificate/srvdokopp/keystorealias)
fi

if test -f /var/run/secrets/nais.io/certificate/srvdokopp/keystorepassword
then
    echo "Setting DOKOPP_CERT_PASSWORD"
    export DOKOPP_CERT_PASSWORD=$(cat /var/run/secrets/nais.io/certificate/srvdokopp/keystorepassword)
fi
