...
public class DokumentproduksjonInfoV1Config extends AbstractCxfEndpointConfig {
	...
	private static final String WSDL_URL = "wsdl/no/nav/tjeneste/domene/brevogarkiv/dokumentproduksjoninfo/v1/dokumentproduksjoninfo.wsdl";
	
	@Bean
	public DokumentproduksjonInfoV1 arkiverDokumentmottakPort(DokumentproduksjonInfoV1Alias dokumentproduksjonInfoV1Alias, ServiceuserAlias serviceuserAlias) {
		setWsdlUrl(WSDL_URL);
		setServiceName(SERVICE_QNAME);
		setEndpointName(PORT_QNAME);
		setAdress(dokumentproduksjonInfoV1Alias.getEndpointurl());
		setReceiveTimeout(dokumentproduksjonInfoV1Alias.getReadtimeoutms());
		setConnectTimeout(dokumentproduksjonInfoV1Alias.getConnecttimeoutms());
		...
		return createPort(DokumentproduksjonInfoV1.class);
	}
